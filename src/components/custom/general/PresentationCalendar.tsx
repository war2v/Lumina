      ...
      <div className="border border-gray-200 rounded-lg overflow-hidden">
        ...
        <div className="grid grid-cols-7 bg-gray-50 dark:bg-gray-950 border-b border-gray-200">
          {dayNames.map((day: string) => (
            <div key={day} className="p-3 text-center text-sm font-medium text-gray-700 dark:text-white">
              {day}
            </div>
          ))}
        </div>
                    ...
                    <div className="space-y-1">
                      {dayEvents.slice(0, 3).map((event: CalendarEvent, eventIndex: number) => (
                        ...
                        >
                          ...
                          <div className="text-red-600">{formatEventTime(event.start_datetime)}</div>
                        </div>
                      ))}
                      {dayEvents.length > 3 && (
        ...
        <div className="flex overflow-scroll w-full">
          {currentMonthEvents
            ...
            .map((event: CalendarEvent) => (
              <div key={event.id} className="p-3 bg-white dark:bg-gray-950 rounded border border-gray-200">
                ...
                <div className="text-sm text-gray-600">
                  {new Date(event.start_datetime).toLocaleDateString('en-US', { 
                    ...
                    month: 'short', 
                    day: 'numeric' 
                  })} at {formatEventTime(event.start_datetime)}
                </div>
                {event.description && (
